...
export const playlistCreate =  async(req:Request,res:Response)=>{
    ...
    try {
        ...
        const playlist = await db.query(`INSERT INTO playlist (name,user_id) VALUES ($1,$2) RETURNING *`,[name,user_id])
        if(!playlist){
...
export const playlistUpdate =  async(req:Request,res:Response)=>{
    try {
        ...
        const result =  await db.query(`UPDATE playlist SET name = $1 WHERE id_playlist = $2 AND user_id = $3 RETURNING *`,[name,id,user_id])
        if(!result){
